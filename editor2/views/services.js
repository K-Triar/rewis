...
export function renderServicesView(container, ctx) {
  ...
  function render() {
    ...
    const lineFilterSelect = h('select', {}, h('option', { value: '' }, '全路線'), ...network.lines.map((l) => h('option', { value: l.id }, l.name)));
    ...
    const categoryFilterSelect = h('select', { disabled: !selectedLine },
      h('option', { value: '' }, '全種別'),
    ...
    container.appendChild(h('div', { class: 'worker-config-actions' },
      h('label', {}, '路線で絞込: ', lineFilterSelect),
  ...
  function renderServiceRow(service) {
    ...
    return h('tr', { class: expandedId === service.id ? 'ed2-row-active' : null },
      ...
      h('td', {},
        ...
        h('button', { class: 'preview-btn', type: 'button', onClick: () => { deletingId = service.id; render(); } }, '削除')
      )
    );
  }
  ...
  function renderServiceForm(service) {
    const isNew = !service;
    const nameInput = h('input', { type: 'text', value: isNew ? '' : (service.name || ''), placeholder: 'メモ（任意）' });
    const headsignInput = h('input', { type: 'text', value: isNew ? '' : (service.headsign || ''), placeholder: '行先' });
    ...
    function renderPreview() {
      ...
      const items = stops.map((stop, i) => {
        ...
        const color = seg ? lineColor(network, seg.lineId) : (nextSeg ? lineColor(network, nextSeg.lineId) : '#999');
        ...
        return h('span', { style: `display:inline-block;padding:2px 6px;margin:2px;border-left:4px solid ${color};background:#f5f5f5;` },
          stationName(network, stop.stationId),
          isBoundary ? h('span', { class: 'ed2-issue-warning' }, ' [直通]') : ''
        );
      });
      previewContainer.appendChild(h('div', {}, ...items));
    }
    ...
    function renderStopRow(stop, index) {
      ...
      const runInput = h('input', { type: 'number', min: '0', step: '1', style: 'width:70px', disabled: !needsRun });
      ...
      const downBtn = h('button', {
        class: 'preview-btn', type: 'button', disabled: index === stops.length - 1,
        onClick: () => {
          [stops[index + 1], stops[index]] = [stops[index], stops[index + 1]];
    ...
    function renderCircularCategory() {
      ...
      circularCategoryContainer.appendChild(h('div', { class: 'worker-config-actions' },
        h('label', {}, '環状区間の路線: ', lineSelect),
        h('label', {}, '種別: ', categorySelect)
      ));
    }
    ...
    return h('div', { class: 'export-card' },
      h('h3', {}, isNew ? '運行系統を追加' : `運行系統を編集: ${service.id}`),
      h('div', { class: 'worker-config-grid' },
        h('label', {}, '行先'), headsignInput,
        h('label', {}, 'メモ'), nameInput,
        h('label', {}, '有効'), activeInput,
        h('label', {}, '環状'), circularInput
      ),
